...
Galtea endpoints execute the same turn service and guardrail as Telegram. The recommended single Conversation connection sends Galtea's documented `galtea_session_id`: an authenticated first message atomically maps its hash to a random internal evaluation UUID and creates the assessment; later turns resume that mapping. Bounded identifiers are validated, empty/unrendered IDs are rejected, and distinct native IDs cannot share a default session. Mapping creation, assessment creation and capacity checks occur in one write transaction. Deletion and retention cleanup remove the mapping; a subsequent request for that native ID creates a fresh internal session. Optional finalization can use the native ID. The explicit UUID-based init/message/finalize contract remains supported, without relaxing its existing-ID requirement. This resolves the onboarding wizard's mandatory conversation test before initialization is available. Configure actual HTTPS Endpoint Connections, not a Python runtime or fake agent stub. Run early before dashboard polish. Freeze cases, evaluator configuration and fresh-session setup; record versions, sample/error counts, real failing cases and the rerun. Security datasets each cover one threat; include factual and benign completion tests to detect blanket refusal. Never fabricate improvement.
