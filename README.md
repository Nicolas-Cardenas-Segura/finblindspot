...
- **A report you can keep**: at the end of every assessment (complete or partial) the bot sends the chat summary *with* the myfinGap-branded PDF attached to that same Telegram message (`myfingap-report-<date>.pdf`, built deterministically in `src/explain/report.ts`; logo in `assets/`, sent as the document's caption, or as a following document when the summary exceeds Telegram's 1024-character caption limit), and the re-assessment reminder question arrives as a separate message afterwards. The report covers: gaps first for partial assessments, projection with the 3/4/5 % sensitivity, assumptions, all blind spots with severity, the top-three action plan with the guarded explanations, the Then / Now comparison on a revisit, the answers as you gave them, and the educational-only disclaimer. Each PDF and its text are stored in the local SQLite `reports` table next to the immutable assessment it belongs to (never sent anywhere else, erased by `/forget`); `/report` resends the latest one, and the text of the last report is placed in Sam's context so the next conversation can refer back to what was actually said.
...
| Command | Behaviour |
| --- | --- |
| `/start` | Shows the education-only disclosure and no-credentials notice, then starts the interview with a saved draft state and walks sections A–H one question at a time. Ends with the results message, the three-item action plan, and a "remind me in 6 or 12 months?" choice that schedules a Telegram nudge. |
